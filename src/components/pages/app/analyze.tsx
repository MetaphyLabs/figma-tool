...
export default function Analyze() {
  ...
  const figmaImageData = useFigmaDesignUploadStore(
    (state) => state.blob
  ) as ImageData;
  const appImageData = useAppDesignUploadStore(
    (state) => state.blob
  ) as ImageData;
  ...
  useEffect(() => {
    if (figmaImageData && appImageData) {
      ...
      setAnalysis(
        "Please upload both the Figma image and the app image for analysis."
      );
    }
